...
一个面向多仓库工作区的跨平台 Git 桌面工作台。适合微服务、前后端分仓、客户定制分支、外包交付仓库很多的团队，把“今天哪些仓库变了、哪些落后了、哪些可以安全 pull”集中到一个清爽窗口里。
...
- 看清：快速发现仓库，展示分支、远端、本地改动、ahead/behind、冲突和 upstream 风险。
- 查清：能在选中仓库里稳定查看 working/staged/HEAD diff 和文件级变更。
- 同步安全：`fetch` 可自动或批量执行，`pull --ff-only` 默认只作用于干净仓库。
- 小步收口：能对当前仓库做明确的 stage/unstage 和 staged commit。
...
- 本地改动、暂存区、远端落后状态分散在不同终端
- 想快速检查某个子仓库的 diff，需要反复切目录
...
- 支持 Working、Staged、HEAD 三种 diff 视图
- 支持点击变更文件查看文件级 diff，未跟踪文本文件也会生成预览
...
.
├── app.go                    # Wails 绑定入口
├── git_service.go            # Git 扫描、状态、diff、fetch/pull、分支逻辑
├── settings.go               # 用户配置持久化
